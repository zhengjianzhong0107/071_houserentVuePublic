...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px 7% 0","margin":"0 0 10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
      ...
      <el-form-item :style='{"margin":"0 4px 0 0"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>租赁状态</div>
        <el-select v-model="formSearch.zulinzhuangtai" placeholder="请选择租赁状态" :clearable="true">
          <el-option v-for="(item, index) in zulinzhuangtaiOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
	  ...
	  <el-button v-if="isAuth('fangwuzulin','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#333","borderRadius":"4px","background":"#f8f089","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/fangwuzulinAdd')"><i v-if="true" :style='{"color":"#333","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"width":"100%","margin":"40px 0 10px","minWidth":"1050px","background":"none"}'>
		...
		<div class="list1 index-pv1" :style='{"padding":"0px","margin":"0 auto","flexWrap":"wrap","background":"none","display":"flex","width":"85%","justifyContent":"space-between","height":"auto"}'>
			<div :style='{"border":"1px solid #e6e6e6","padding":"10px","margin":"0 0 20px","flexWrap":"wrap","background":"rgba(255,255,255,.8)","display":"flex","width":"23.5%","position":"relative","justifyContent":"space-between","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				<img :style='{"width":"100%","boxShadow":"0px 56px 36px -60px #999","objectFit":"cover","borderRadius":"4px","display":"block","height":"275px"}' v-if="item.fangwufengmian && item.fangwufengmian.substr(0,4)=='http'" :src="item.fangwufengmian" class="image" />
				<img :style='{"width":"100%","boxShadow":"0px 56px 36px -60px #999","objectFit":"cover","borderRadius":"4px","display":"block","height":"275px"}' v-else :src="baseUrl + (item.fangwufengmian?item.fangwufengmian.split(',')[0]:'')" class="image" />
				...
				<div :style='{"padding":"4px 10px","lineHeight":"32px","fontSize":"14px","color":"#3d74c0"}' class="name ">租赁状态:{{item.zulinzhuangtai}}</div>
			</div>
		</div>
...
<script>
  export default {
    ...
    methods: {
      ...
      getFenlei() {
        this.$http.get('option/fangyuanfenlei/fangyuanfenlei').then(res => {
          if (res.data.code == 0) {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.zulinzhuangtai != '') searchWhere.zulinzhuangtai = this.formSearch.zulinzhuangtai;
	      params['sfsh'] = '是';
        if (this.curFenlei != '全部') searchWhere.fangyuanfenlei = this.curFenlei;
        this.$http.get('fangwuzulin/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/fangwuzulinDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
