...
<div :style='{"width":"85%","padding":"20px","margin":"10px auto","position":"relative","background":"rgba(255,255,255,0.8)"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="房主姓名" prop="fangzhuxingming">
            <el-input v-model="ruleForm.fangzhuxingming" 
                placeholder="房主姓名" clearable ></el-input>
          </el-form-item>
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('fangzhu/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
                  ...
                  params: params
                     ...
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('fangzhu/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
